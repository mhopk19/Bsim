...
if __name__ == '__main__':
    ...
    data = bat.create_data(batch_timesteps = timesteps, total_time = 60000, dt = 0.1)
    ...
    vrnn_data = np.zeros((num_batches, timesteps-10, 2 * prev_inputs + 1))
    ...
    for j in range(num_batches):
        for i in range(timesteps-10):
            ...
            """
            vrnn_data[j][i][0:10] = data[j][0][i:i+10]
            vrnn_data[j][i][10:20] = data[j][1][i:i+10]
            vrnn_data[j][i][20] = data[j][2][i]
           
        plt.cla()
        ax[1].plot(range(timesteps-10),vrnn_data[j,:,9])
        ax[1].plot(range(timesteps-10),vrnn_data[j,:,19])
        ax[1].plot(range(timesteps-10),vrnn_data[j,:,20])    
        plt.pause(0.01) 
    ...
    vrnn_data = np.expand_dims(vrnn_data, axis=1)   
